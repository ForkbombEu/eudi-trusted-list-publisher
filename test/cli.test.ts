...
/*
  Every case here spawns `tsx` and transpiles the CLI's import graph in a fresh
  process, which costs several seconds before the command itself runs. The
  default 5 s budget is shorter than that startup on a loaded machine, so these
  tests fail for their runtime rather than their behaviour. The timeout states
...
describe("CLI", () => {
  describe("compile", () => {
    it(
      "compiles valid input to stdout",
      () => {
    ...
    it(
      "rejects invalid input with exit code 2",
      () => {
        const result = runCli([
          "compile",
          "-i",
  ...
  describe("validate", () => {
    ...
    it(
      "validates compiled ETSI structure",
      () => {
    ...
    it(
      "fails on invalid ETSI structure",
      () => {
  ...
  describe("verify", () => {
    it(
      "verifies a compact JAdES signature",
      () => {
    ...
    it(
      "rejects invalid format with exit code 1",
      () => {
    ...
    it(
      "verifies detached format",
      () => {
        const compileResult = runCli(["compile", "-i", SCHEME_PATH]);
        expect(compileResult.status).toBe(0);
        const compiledPath = resolve(
          tmpdir(),
          `test-det-c-${randomUUID()}.json`,
        );
        const detachedPath = resolve(
          tmpdir(),
          `test-det-d-${randomUUID()}.json`,
        );
        try {
          writeFileSync(compiledPath, compileResult.stdout, "utf-8");
          const signResult = runCli([
  ...
  describe("publish", () => {
    it(
      "publishes a valid signed LoTE and exits 0",
      () => {
        const compileResult = runCli(["compile", "-i", SCHEME_PATH]);
        expect(compileResult.status).toBe(0);
        const signedPath = resolve(
          tmpdir(),
          `test-pub-cli-${randomUUID()}.txt`,
        );
    ...
    it(
      "fails with non-zero exit on invalid signature",
      () => {
        const compiledPath = resolve(
          tmpdir(),
        ...
        const outPath = resolve(
          tmpdir(),
          `test-badsig-out-${randomUUID()}.txt`,
        );
