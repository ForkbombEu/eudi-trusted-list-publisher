...
/**
 * Removes material generated by this administration UI for one list. Material
 * supplied from another location is intentionally outside this operation.
 */
export function deleteGeneratedSigningMaterial(
  certificatesDir: string,
  listKey: string,
): boolean {
  const root = resolve(certificatesDir);
  const directory = resolve(root, listKey);
  if (!directory.startsWith(`${root}${sep}`))
    throw new Error(
      "Generated signing material path escapes its configured root.",
    );
  if (!existsSync(directory)) return false;
  if (lstatSync(directory).isSymbolicLink())
    throw new Error("Generated signing material directory is a symlink.");
